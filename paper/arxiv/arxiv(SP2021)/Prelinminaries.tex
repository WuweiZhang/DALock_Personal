...
\begin{definition}[count sketch~\cite{ICALP:ChaCheFar02,JoA:CorMut05}]
...
\mypara{$\sigma_{new} \leftarrow $ Add($pw, \sigma$):} Add operation increases the stored frequency count of password $pw$ by 1 based on count sketch state $\sigma$ and outputs the updated state $\sigma_{new}$.
