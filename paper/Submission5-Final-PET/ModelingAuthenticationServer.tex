...
\mypara{Memory Efficiency} We instantiate the count sketch with parameters $d=5$ and $w=10^6$ so that the entire data structure requires just $20$ MB of space, which easily fits in modern RAM. 
...
% {We assume that the attacker may have the ability to obtained a snapshot of the authentication server and A differentially private count-sketch can protect the privacy of individual user passwords under this case. If the attacker can continuously observe the internal state of the authentication server and obtain multiple snapshots then the server need a stronger privacy notion such as Pan-Privacy\cite{dwork2010pan}.  If we want to protect against an attacker who  If the attacker is continuously present on the authentication server they would presumably also be able to see all of the plaintext passwords for newly registered accounts. }
