...
def H_w(x,i,w):
    ...
    #print(type(i))
    temp=temp % w
    #print('col for ',x,i,' will be ',temp)
    return temp
def Hash1(x,i):
    result =-1
    b=bin(int.from_bytes(x.encode(), 'big'))    #x=x|i
...
#total needs to be replaced with array of hashes from each col, then return min from this array
    ...
    for i in range(1,d):
        col=H_w(x,i,w)
        total.append(matrix_a[i][col]*Hash1(x,i))
...
for line in atfile:
   ...
   # if len(items)==2:
    ...
    if ((items[4] and items[5])=='0' and items[2]!='-1'):
        ...
        #print(items[1],'curr')
        userattempts[int(items[1])].append(items[3])
        #print(userattempts)
            #print(curr,'curr')
        ...
        if items[2]!='-1':
            ...
            if items[3] not in userattempts[int(items[1])]:
            ...
            #print('userattempts again ', userattempts)
                ...
                # else:
                    # index1=0.5
                #print('estimate ',items[3], ' ',EstimateSketchCount(items[3]),'at time ',items[2])
                ...
                print(curr, items[3], index1)
                    #print(df)
                #print(df.loc[curr-1])

df.to_csv('system1primeAttackCountMin.csv')
print('done')