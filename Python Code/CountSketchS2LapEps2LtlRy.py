...
cumerror=0
cumerrorlap=0
file = open('littlerockyouNOISEEps2.out','r')
matrix_an=np.genfromtxt('littlerockyouNOISEEps2.out', delimiter=' ', usecols=range(3000))
for line in atfile:
    line=line.strip()
    #print(line)
    items=line.split(',')
    ...
    if ((items[4] and items[5])=='0' and items[2]!='-1'):
        ...
        #print(userattempts)
            ...
            print(curr, items[2], index1)
                    #print(df)
                #print(df.loc[curr-1])
df.to_csv('countsketchsys2primeLAPLACEeps2.csv')
print('done')