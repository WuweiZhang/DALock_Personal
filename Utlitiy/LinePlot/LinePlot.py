...
kAxisIndex= [5, 20]
CaptchaDiction = [float('Inf'),180*5,50]
thisPath = "."
...
for datafile in files:
	...
	if datafile[0] == '.' or '.py' in datafile or '.png' in datafile:
		...
		with open(datafile) as f:
			...
			for ratio in GAINPERACCOUNTOVERCAPTCHA:
				for k in CrackDiction:
					newL = []
					for i in range(0,len(CrackDiction[k])):
						if (float(GuessDiction[k][i]) == 0):
							...
							if (float(CaptchaDiction[k][i]) == 0):
								newL.append(float(CrackDiction[k][i]) * 65424 * GAINPERACCOUNTOVERCAPTCHA)
							else:
								newL.append(float(CrackDiction[k][i])/float(float(CaptchaDiction[k][i]) == 0) *65424 * GAINPERACCOUNTOVERCAPTCHA )
					plt.plot(days,newL, label = k)
				plt.legend(fontsize='small')
				plt.xlabel('Time (Days)')
				plt.ylabel('GainPerUser/CostPerCaptCha')
				plt.title('Economic Gain Over 180 Days' + str(datafile)+"_ratio_"+str(ratio))
				plt.savefig(datafile +"_ratio_"+str(ratio) +'_eco.png')
				plt.clf()
